...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<Bitcoin 3.0Unit>("Bitcoin 3.0Unit");
#else
    qRegisterMetaType<Bitcoin 3.0Unit>("Bitcoin 3.0Unit");
#endif
}
...
Bitcoin 3.0Application::Bitcoin 3.0Application()
...
void Bitcoin 3.0Application::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the Bitcoin 3.0Application constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", Bitcoin 3.0GUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void Bitcoin 3.0Application::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool Bitcoin 3.0Application::createOptionsModel(bool resetSettings)
{
...
void Bitcoin 3.0Application::createWindow(const NetworkStyle *networkStyle)
{
    window = new Bitcoin 3.0GUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &Bitcoin 3.0GUI::quitRequested, this, &Bitcoin 3.0Application::requestShutdown);
...
void Bitcoin 3.0Application::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void Bitcoin 3.0Application::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool Bitcoin 3.0Application::baseInitialize()
{
    return node().baseInitialize();
}

void Bitcoin 3.0Application::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &Bitcoin 3.0Application::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &Bitcoin 3.0Application::handleRunawayException);
    connect(this, &Bitcoin 3.0Application::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &Bitcoin 3.0Application::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void Bitcoin 3.0Application::parameterSetup()
{
...
void Bitcoin 3.0Application::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void Bitcoin 3.0Application::requestInitialize()
{
...
void Bitcoin 3.0Application::requestShutdown()
{
...
void Bitcoin 3.0Application::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if(success) {
        ...
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &Bitcoin 3.0GUI::handlePaymentRequest);
            connect(window, &Bitcoin 3.0GUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void Bitcoin 3.0Application::handleRunawayException(const QString &message)
{
...
void Bitcoin 3.0Application::handleNonFatalException(const QString& message)
{
...
WId Bitcoin 3.0Application::getMainWinId() const
{
    if (!window)
        return 0;
...
bool Bitcoin 3.0Application::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", Bitcoin 3.0GUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
