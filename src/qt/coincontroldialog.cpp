...
{
    GUIUtil::setClipboard(Bitcoin 3.0Units::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::updateLabels(CCoinControl& m_coin_control, WalletModel *model, QDialog* dialog)
{
    ...
    Bitcoin 3.0Unit nDisplayUnit = Bitcoin 3.0Unit::BT3;
    ...
    l2->setText(Bitcoin 3.0Units::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(Bitcoin 3.0Units::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(Bitcoin 3.0Units::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));        // Bytes
    l7->setText(fDust ? tr("yes") : tr("no"));                               // Dust
    l8->setText(Bitcoin 3.0Units::formatWithUnit(nDisplayUnit, nChange));        // Change
    if (nPayFee > 0)
    {
...
void CoinControlDialog::updateView()
{
    ...
    Bitcoin 3.0Unit nDisplayUnit = model->getOptionsModel()->getDisplayUnit();
    ...
    for (const auto& coins : model->wallet().listCoins()) {
        ...
        for (const auto& outpair : coins.second) {
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, Bitcoin 3.0Units::format(nDisplayUnit, out.txout.nValue));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, Bitcoin 3.0Units::format(nDisplayUnit, nSum));
