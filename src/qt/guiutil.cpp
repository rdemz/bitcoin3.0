...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Bitcoin 3.0 address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new Bitcoin 3.0AddressEntryValidator(parent));
    widget->setCheckValidator(new Bitcoin 3.0AddressCheckValidator(parent));
}
...
bool parseBitcoin 3.0URI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!Bitcoin 3.0Units::parse(Bitcoin 3.0Unit::BT3, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseBitcoin 3.0URI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseBitcoin 3.0URI(uriInstance, out);
}

QString formatBitcoin 3.0URI(const SendCoinsRecipient &info)
{
    ...
    if (info.amount)
    {
        ret += QString("?amount=%1").arg(Bitcoin 3.0Units::format(Bitcoin 3.0Unit::BT3, info.amount, false, Bitcoin 3.0Units::SeparatorStyle::NEVER));
        paramCount++;
    }
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Bitcoin 3.0.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Bitcoin 3.0 (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Bitcoin 3.0 (%s).lnk", chain));
}

bool GetStartOnSystemStartup()
{
    // check for Bitcoin 3.0*.lnk
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Bitcoin 3.0\n";
        else
            optionFile << strprintf("Name=Bitcoin 3.0 (%s)\n", chain);
