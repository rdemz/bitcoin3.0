...
/** Call node.updateRwSetting() with Bitcoin 3.0 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const util::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Bitcoin 3.0 22.x releases try to read these specific settings as
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayBitcoin 3.0Unit")) {
        settings.setValue("DisplayBitcoin 3.0Unit", QVariant::fromValue(Bitcoin 3.0Unit::BT3));
    }
    QVariant unit = settings.value("DisplayBitcoin 3.0Unit");
    if (unit.canConvert<Bitcoin 3.0Unit>()) {
        m_display_bitcoin_unit = unit.value<Bitcoin 3.0Unit>();
    } else {
        m_display_bitcoin_unit = Bitcoin 3.0Unit::BT3;
        settings.setValue("DisplayBitcoin 3.0Unit", QVariant::fromValue(m_display_bitcoin_unit));
    }
...
{
    if (new_unit.isNull() || new_unit.value<Bitcoin 3.0Unit>() == m_display_bitcoin_unit) return;
    m_display_bitcoin_unit = new_unit.value<Bitcoin 3.0Unit>();
    QSettings settings;
    settings.setValue("DisplayBitcoin 3.0Unit", QVariant::fromValue(m_display_bitcoin_unit));
