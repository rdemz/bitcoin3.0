...
def assert_fee_amount(fee, tx_size, feerate_BT3_kvB):
    """Assert the fee is in range."""
    assert isinstance(tx_size, int)
    target_fee = get_fee(tx_size, feerate_BT3_kvB)
    if fee < target_fee:
        raise AssertionError("Fee of %s BT3 too low! (Should be %s BT3)" % (str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    high_fee = get_fee(tx_size + 2, feerate_BT3_kvB)
    if fee > high_fee:
        raise AssertionError("Fee of %s BT3 too high! (Should be %s BT3)" % (str(fee), str(target_fee)))
...
def check_json_precision():
    """Make sure json library being used does not lose precision converting BT3 values"""
...
def get_fee(tx_size, feerate_btc_kvb):
    """Calculate the fee in BT3 given a feerate is BT3/kvB. Reflects CFeeRate::GetFee"""
    feerate_sat_kvb = int(feerate_btc_kvb * Decimal(1e8)) # Fee in sat/kvb as an int to avoid float precision errors
    target_fee_sat = ceildiv(feerate_sat_kvb * tx_size, 1000) # Round calculated fee up to nearest sat
    return target_fee_sat / Decimal(1e8) # Return result in  BT3
...
def wait_until_helper(predicate, *, attempts=float('inf'), timeout=float('inf'), lock=None, timeout_factor=1.0):
    ...
    from `Bitcoin 3.0TestFramework` or `P2PInterface` class ensures the timeout is
