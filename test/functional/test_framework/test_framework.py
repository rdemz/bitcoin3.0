...
class Bitcoin 3.0TestMetaClass(type):
    """Metaclass for Bitcoin 3.0TestFramework.

    Ensures that any attempt to register a subclass of `Bitcoin 3.0TestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'Bitcoin 3.0TestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("Bitcoin 3.0TestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("Bitcoin 3.0TestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class Bitcoin 3.0TestFramework(metaclass=Bitcoin 3.0TestMetaClass):
    ...
    def shutdown(self):
        ...
        rpc_logger = logging.getLogger("Bitcoin 3.0RPC")
