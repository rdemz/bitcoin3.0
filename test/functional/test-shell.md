...
The `TestShell` submodule extends the `Bitcoin 3.0TestFramework` functionality to
external interactive environments for prototyping and educational purposes. Just
like `Bitcoin 3.0TestFramework`, the `TestShell` allows the user to:
...
to extend the object lifetime of the underlying `Bitcoin 3.0TestFramework` between
...
## 2. Importing `TestShell` from the Bitcoin 3.0 Core repository

We can import the `TestShell` by adding the path of the Bitcoin 3.0 Core
...
The `TestShell` inherits all `Bitcoin 3.0TestFramework` members and methods, such
as:
...
`Bitcoin 3.0TestFramework` object. The full set of argument keywords which can be
...
Unlike the `Bitcoin 3.0TestFramework` class, the `TestShell` keeps the underlying
Bitcoin 3.0d subprocesses (nodes) and logging utilities running until the user
explicitly shuts down the `TestShell` object.

During the time between the `setup` and `shutdown` calls, all `bitcoind` node
processes and `Bitcoin 3.0TestFramework` convenience methods can be accessed
interactively.
...
underlying `Bitcoin 3.0TestFramework`:
...
`Bitcoin 3.0TestFramework` class. The user can override these in
