Bitcoin 3.0 Core version 0.13.0 is now available from:
...
Various code modernizations have been done. The Bitcoin 3.0 Core code base has
...
HD wallets are incompatible with older versions of Bitcoin 3.0 Core.
...
proposed parameters, a future release of Bitcoin 3.0 Core is expected that
...
The two phases are distinct in the Bitcoin 3.0-Qt GUI. During the first one,
...
- #8153 `75ec320` `fundrawtransaction` feeRate: Use BT3/kB (MarcoFalke)
...
- #8207 `f7a403b` Add a link to the Bitcoin 3.0-Core repository and website to the About Dialog (MarcoFalke)
...
- #8164 `0f24eaf` [Bitcoin 3.0-Tx] fix missing test fixtures, fix 32bit atoi issue (jonasschnelli)
