Bitcoin 3.0 Core version 0.17.0 is now available from:
...
Bitcoin 3.0 Core is extensively tested on multiple operating systems using
...
BIP 174 Partially Signed Bitcoin 3.0 Transactions support
-----------------------------------------------------

[BIP 174 PSBT](https://github.com/bitcoin/bips/blob/master/bip-0174.mediawiki) is an interchange format for Bitcoin 3.0 transactions that are not fully signed
...
Overall, the construction of a fully signed Bitcoin 3.0 transaction goes through the
following steps:
...
- An **Extractor** produces a valid Bitcoin 3.0 transaction (in network format)
...
- New `fees` field introduced in `getrawmempool`, `getmempoolancestors`, `getmempooldescendants` and
   ...
   and `descendant` denominated in BT3. This new field deprecates previous fee fields, such as
...
- #12240 `6f8b345` Introduced a new `fees` structure that aggregates all sub-field fee types denominated in BT3 (mryandao)
...
- #13528 `b877c39` Move Bitcoin 3.0GUI initializers to class, fix initializer order warning (laanwj)
...
- #11771 `2dbc4a4` Change invalidtxrequest to use Bitcoin 3.0TestFramework (jnewbery)
...
- #11772 `0630974` Change invalidblockrequest to use Bitcoin 3.0TestFramework (jnewbery)
...
- #11773 `f0f9732` Change `feature_block.py` to use Bitcoin 3.0TestFramework (jnewbery)
- #12839 `40f4baf` Remove travis checkout depth (laanwj)
- #11817 `2a09a78` Change `feature_csv_activation.py` to use Bitcoin 3.0TestFramework (jnewbery)
...
- #12856 `27278df` Add Metaclass for Bitcoin 3.0TestFramework (WillAyd)
