...
Bitcoin 3.0 Core version 24.0.1 is now available from:
...
installer (on Windows) or just copy over `/Applications/Bitcoin 3.0-Qt` (on macOS)
...
This version of Bitcoin 3.0 Core adds a new `mempoolfullrbf` configuration
...
Some Bitcoin 3.0 services today expect that the first version of an
...
The Bitcoin 3.0 Protocol does not, and cannot, provide any assurance that
...
fees, was realized in [Bitcoin 3.0 Core 0.12.0][] (February 2016) with the
...
default for years, and several contributors to Bitcoin 3.0 Core are
...
[Bitcoin 3.0 Core 0.12.0]: https://bitcoincore.org/en/releases/0.12.0/#opt-in-replace-by-fee-transactions
...
- The `wsh()` output descriptor was extended with Miniscript support. You can import Miniscript
  ...
  the Bitcoin 3.0 Core wallet yet.
...
- To help prevent fingerprinting transactions created by the Bitcoin 3.0 Core wallet, change output
