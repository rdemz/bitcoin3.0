Bitcoin 3.0 Core version 0.18.0 is now available from:
...
`/Applications/Bitcoin 3.0-Qt` (on Mac) or `bitcoind`/`bitcoin-qt` (on
Linux).
...
Bitcoin 3.0 Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Bitcoin 3.0 Core on unsupported systems.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://bitcoincore.org/en/contact/) the Bitcoin 3.0 Core
...
`~bitcoin/.bitcoin`. This change makes Bitcoin 3.0 Core more consistent with
...
- A new document introduces Bitcoin 3.0 Core's BIP174 [Partially-Signed
  Bitcoin 3.0 Transactions
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Bitcoin 3.0-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Bitcoin 3.0 Core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Bitcoin 3.0 Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Bitcoin 3.0 Core, subsequently importing
...
- A new `bitcoin-wallet` tool is now distributed alongside Bitcoin 3.0
...
This section describes planned changes to Bitcoin 3.0 Core that may affect
other Bitcoin 3.0 software and services.

- Since version 0.16.0, Bitcoin 3.0 Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Bitcoin 3.0 Core 0.20 (expected about
  a year after 0.18), Bitcoin 3.0 Core will default to native segwit
  ...
  to bech32 addresses, and if the Bitcoin 3.0 Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Bitcoin 3.0 Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Bitcoin 3.0 Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Bitcoin 3.0 Core
...
- A sub-project of Bitcoin 3.0 Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Bitcoin 3.0 Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Bitcoin 3.0 Core's own implementation, although entropy
  gathered by Bitcoin 3.0 Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Bitcoin 3.0 Core a
...
- On macOS, Bitcoin 3.0 Core now opts out of application CPU throttling
...
- #11625 Add Bitcoin 3.0Application & RPCConsole tests (ryanofsky)
...
- #11526 Visual Studio build configuration for Bitcoin 3.0 Core (sipsorcery)
...
- #12775 Integration of property based testing into Bitcoin 3.0 Core (Christewart)
