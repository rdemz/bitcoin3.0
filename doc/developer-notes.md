...
- [Development guidelines](#development-guidelines)
    - [General Bitcoin 3.0 Core](#general-bitcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Bitcoin 3.0 Core uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
Bitcoin 3.0 Core is a multi-threaded application, and deadlocks or other
...
Bitcoin 3.0 Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as Bitcoin 3.0 Core, where everyone uses
...
pay attention to for reviewers of Bitcoin 3.0 Core code.

General Bitcoin 3.0 Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: Bitcoin 3.0 Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Some of these are maintained by active developers of Bitcoin 3.0 Core, in which case
...
a Bitcoin 3.0 Core subtree, so that they can be integrated quickly. Cosmetic changes
...
file descriptors, it will cause problems with Bitcoin 3.0's `select()` loop, because
...
compatibility between nodes. This happened in Bitcoin 3.0 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to Bitcoin 3.0's
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Bitcoin 3.0 Core, the wallet was always
