When Bitcoin 3.0 Core automatically opens outgoing P2P connections, it chooses
...
A malicious actor may gossip an address:port where no Bitcoin 3.0 node is listening,
or one where a service is listening that is not related to the Bitcoin 3.0 network.
As a result, this service may occasionally get connection attempts from Bitcoin 3.0
nodes.

"Bad" ports are ones used by services which are usually not open to the public
and usually require authentication. A connection attempt (by Bitcoin 3.0 Core,
trying to connect because it thinks there is a Bitcoin 3.0 node on that
...
Below is a list of "bad" ports which Bitcoin 3.0 Core avoids when choosing a peer to
