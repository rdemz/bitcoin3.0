...
The data directory is the default location where the Bitcoin 3.0 Core files are stored.
...
---------|--------------------
Linux    | `$HOME/.bitcoin/`
macOS    | `$HOME/Library/Application Support/Bitcoin 3.0/`
Windows  | `%APPDATA%\Bitcoin 3.0\` <sup>[\[1\]](#note1)</sup>
...
`blocks/`          | `blkNNNNN.dat`<sup>[\[2\]](#note2)</sup> | Actual Bitcoin 3.0 blocks (in network format, dumped in raw on disk, 128 MiB per file)
...
These subdirectories and files are no longer used by Bitcoin 3.0 Core:
