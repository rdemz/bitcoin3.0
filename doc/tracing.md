# User-space, Statically Defined Tracing (USDT) for Bitcoin 3.0 Core

Bitcoin 3.0 Core includes statically defined tracepoints to allow for more
...
**Note**: Bitcoin 3.0 Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Bitcoin 3.0 Core
...
To list probes in Bitcoin 3.0 Core, use `info probes` in `gdb`:
...
The `readelf` tool can be used to display the USDT tracepoints in Bitcoin 3.0 Core.
